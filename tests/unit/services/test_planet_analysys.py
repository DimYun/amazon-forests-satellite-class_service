...
class FakePlanetClassifier:
    def process(self, image, content_id):
        return ('haze: 0, primary: 0, agriculture: 0, clear: 0, water: 0, '
...
def test_prob_less_or_equal_to_one(app_container: Container, sample_image_np: np.ndarray):
    with app_container.reset_singletons():
        with app_container.content_process.override(FakePlanetClassifier()):
            planet_analytics = app_container.content_process()
...
def test_predict_dont_mutate_initial_image(app_container: Container, sample_image_np: np.ndarray):
    with app_container.reset_singletons():
        with app_container.content_process.override(FakePlanetClassifier()):
